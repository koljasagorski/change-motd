...
# Funktion zur Anzeige des Tor- und Nyx-Status
function display_tor_status {
    if systemctl is-active --quiet tor; then
        echo -e "\e[32mTor ist aktiv\e[0m"
        
        # Prüfen auf Tor Control Port-Verbindung
        if command -v nc &> /dev/null; then
            TOR_CONTROL_PORT=9051
            TOR_PASSWORD="password"  # Passwort hier ersetzen oder über eine sichere Methode beziehen
            AUTH_COMMAND="AUTHENTICATE \"${TOR_PASSWORD}\""
            
            # Teste Verbindung zum Tor-Controlport
            if echo "$AUTH_COMMAND" | nc 127.0.0.1 $TOR_CONTROL_PORT; then
                # Hole Tor-Informationen
                echo "Tor-Informationen:"
                echo "------------------"
                echo -e "Circuit-Build-Time: $(echo "GETINFO status/circuit-established" | nc 127.0.0.1 $TOR_CONTROL_PORT | grep '1' &> /dev/null && echo 'Erfolgreich' || echo 'Fehlgeschlagen')"
                echo -e "Aktive Tor-Verbindungen: $(echo "GETINFO circuit-status" | nc 127.0.0.1 $TOR_CONTROL_PORT | grep -c '^')"
            else
                echo -e "\e[31mVerbindung zum Tor-Controlport fehlgeschlagen. Kontrollieren Sie die Einstellungen.\e[0m"
            fi
        else
            echo -e "\e[31mnc (netcat) ist nicht installiert. Installieren Sie es, um Tor-Informationen abzurufen.\e[0m"
        fi
    else
        echo -e "\e[31mTor ist inaktiv\e[0m"
    fi
}
...
echo "Tor- und Nyx-Status:" | sudo tee -a $MOTD_FILE
echo "---------------------" | sudo tee -a $MOTD_FILE
display_tor_status | sudo tee -a $MOTD_FILE
echo "" | sudo tee -a $MOTD_FILE
